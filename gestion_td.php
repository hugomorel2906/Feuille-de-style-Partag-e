...
<head>
  ...
  <title>Page Banque de Questions</title>
...
if(isset($_GET["cadre2"])){  //Condition sur l'existence du bouton submit 
 ...
 if(isset($_GET["bt_valider_nouv_categorie"])){
				...
				$SQL_id_question="SELECT id_question FROM question WHERE numero_question='$value'";
				$result_id_question= mysqli_query($link, $SQL_id_question); //envois de la requete au serveur 
				$tab_id_question= mysqli_fetch_all($result_id_question);  //Mise en forme tableau du résultat
				$val_id_question=$tab_id_question[0][0]; //on récupère la valeur dans une variable 
                $querynew="INSERT INTO categorie (lib_categorie) VALUES ('".$nouvelle_categorie."')";
                $envoisnew= mysqli_query($link, $querynew);
                $querymaj="UPDATE categorie SET categorie.lib_categorie='".$nouvelle_categorie."' WHERE categorie.id_question='$val_id_question'";
...
//DEUXIEME CADRE
 ...
 echo'<br><br><form method="GET" action="gestion_td.php">';
		//Code nom question 
	
$val=$_GET['bouton1'];
$query20= "SELECT id_question FROM question WHERE numero_question='$val'"; 
$result20= mysqli_query($link,$query20);
$tab20=mysqli_fetch_all($result20); 
$oui= $tab20[0][0] ; 


$query5= "SELECT numero_question FROM question WHERE id_question='$oui'";
